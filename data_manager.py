...
from constants import HIGH_SCORE_FILE, MONEY_SCORE_FILE, SHOP_DATA_FILE
...
def load_money_score() -> int:
    """Загружает количество монет из файла"""
...
def save_high_score() -> None:
    ...
    data: Dict[str, int] = {"high_score": int(global_high_score)}
...
def save_money_score() -> None:
    """Сохраняет количество монет в файл"""
    global global_money_score, money_updated

    if not money_updated:
        return

    data: Dict[str, int] = {"money_score": int(global_money_score)}
...
def update_high_score(new_score: Union[int, float]) -> int:
    """Обновляет рекорд"""
...
def update_money_score(new_money_score: Union[int, float]) -> int:
...
def load_shop_data() -> Dict[str, Any]:
...
def save_shop_data(data: Dict[str, Any]) -> bool:
    """Сохраняет данные магазина в файл"""
...
def save_game_state(menu: Any, selected_accessory: int) -> None:
