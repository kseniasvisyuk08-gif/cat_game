...
def main():
    ...
    # Создание меню
    game_menu = GameMenu(cat_stand, cat_jump, cat_cloud)
    game_menu.load_data(shop_data)
    game_menu.coins = global_money_score

    # Создание кота
    cat = Cat(cat_stand, cat_jump, cat_cloud, selected_accessory_type)
    ...
    score = 0
    current_coins = 0
    total_coins = global_money_score
    food = 0
    milk = 0
    mice_killed = 0
    ...
    while running:
                    ...
                    if game_menu.active:
                        if game_menu.show_instructions:
                            ...
                            if len(available_items) <= 1:
                                ...
                                button_height = 50
                                button_y = 200
                                shop_rect = pygame.Rect(SCREEN_WIDTH // 2 - button_width // 2,
                                ...
                                for i, (name, accessory_id, item_id) in enumerate(available_items):
                                    ...
                                    button_rect = pygame.Rect(card_x + 20, y_pos + card_height - 40,
                                                              card_width - 40, 30)
                                ...
                                continue_rect = pygame.Rect(buttons_start_x, button_y,
                                                            button_width, button_height)
                                shop_rect = pygame.Rect(buttons_start_x + button_width + button_spacing,
                                                        button_y, button_width, button_height)
        ...
        if not game_over and not game_paused and not game_menu.active:
            ...
            if spawn_timer >= 90:
                ...
                if random.random() < 0.15 and Obstacle.can_spawn_group(last_obstacle.rect if last_obstacle else None):
                    ...
                    for i in range(2):
                        obstacle = Obstacle(pregrada_images, pregrada_types, is_group=True)
        ...
        # Инструкции слева
        instructions_left_x = 15
        instructions_start_y = 20
        instructions_spacing = 22
